...
void MissileEffect::show(RECT rc)
{
	...
	effect.img->init("Resource/Images/Rocket/Effect.bmp", 0.0f, 0.0f, 390, 63, 6, 1, true, RGB(50, 150, 200));
...
{
	SAFE_DELETE(_viEffect[arrNum].img);
...
void BeamEffect::show(RECT rc)
{
	...
	effect.img = new my::Image;
	effect.img->init("Resource/Images/Rocket/BeamEffect.bmp", 0.0f, 0.0f, 96, 128, 4, 1, true, RGB(255, 0, 255));
	effect.x = rc.left + (rc.right - rc.left) / 2;
	effect.y = rc.top;
	effect.rc = RectMakeCenter(effect.x, effect.y, effect.img->getFrameWidth(), effect.img->getFrameHeight());
